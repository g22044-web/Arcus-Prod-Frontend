    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $.each(empAllData, function (i, empData) {
                    empIndData = empData.BENCH_DASHBOARD_DATA;
            });
...
function getEmpDataTable(emp) {
    ...
    for (var i = 0;i < emp.length;i++) {
        let emp_skills = emp[i].SKILL_DATA;
        let emp_skill_data = "";
        ...
        let NoticeHover = "";
        if (emp[i].END_DATE != "0000-00-00") {
            NoticeHover = (emp[i].END_DATE == "0000-00-00" ? "" : convert(emp[i].END_DATE))
        }


        var row = $('<tr><td>' + emp[i].EMPLOYEE_ID + '</td><td>' +
            emp[i].EMPLOYEE_NAME + '</td><td>' +
            emp[i].REPORTING_MANAGER + '</td><td>' +
            emp[i].LOCATION + '</td><td class="more">' +
            emp_skill_data + '</td><td>' +
            emp[i].JOB_ROLE_ID + '</td><td>' +
            (emp[i].CURRENT_CUSTOMER_NAME == null ? "" : emp[i].CURRENT_CUSTOMER_NAME) + '</td><td>' +
            (emp[i].CURRENT_SOW_NAME == null ? "" : emp[i].CURRENT_SOW_NAME) + '</td><td>' +
            (emp[i].CURRENT_STATUS == null ? "" : emp[i].CURRENT_STATUS) + '</td><td>' +
            (emp[i].CURRENT_START_DATE == null ? "" : convert(emp[i].CURRENT_START_DATE)) + '</td><td>' +
            (emp[i].CURRENT_END_DATE == null ? "" : convert(emp[i].CURRENT_END_DATE)) + '</td><td>' +
            (emp[i].PREV_CUSTOMER_NAME == null ? "" : emp[i].PREV_CUSTOMER_NAME) + '</td><td>' +
            (emp[i].PREV_SOW_NAME == null ? "" : emp[i].PREV_SOW_NAME) + '</td><td>' +
            (emp[i].PREV_STATUS == null ? "" : emp[i].PREV_STATUS) + '</td><td>' +
            (emp[i].PREV_START_DATE == null ? "" : convert(emp[i].PREV_START_DATE)) + '</td><td>' +
            (emp[i].PREV_END_DATE == null ? "" : convert(emp[i].PREV_END_DATE)) + '</td><td>' +
            (emp[i].NEXT_CUSTOMER_NAME == null ? "" : emp[i].NEXT_CUSTOMER_NAME) + '</td><td>' +
            (emp[i].NEXT_SOW_NAME == null ? "" : emp[i].NEXT_SOW_NAME) + '</td><td>' +
            (emp[i].NEXT_STATUS == null ? "" : emp[i].NEXT_STATUS) + '</td><td>' +
            (emp[i].NEXT_START_DATE == null ? "" : convert(emp[i].NEXT_START_DATE)) + '</td><td>' +
            (emp[i].NEXT_END_DATE == null ? "" : convert(emp[i].NEXT_END_DATE)) + '</td><td style="display: none">' +
...
function filterData() {
    ...
    if (jobSelectArray.length > 0) {
        newJson = newJson.filter(d => {
            const obj = jobSelectArray.find(f => d.JOB_ROLE_ID == f);
            return obj ? true : false;
        })
    }
    if (managerSelectArray.length > 0) {
        newJson = newJson.filter(d => {
            const obj = managerSelectArray.find(f => d.REPORTING_MANAGER == f);
            return obj ? true : false;
        })
    }
    if (locSelectArray.length > 0) {
        newJson = newJson.filter(d => {
            const obj = locSelectArray.find(f => d.LOCATION == f);
            return obj ? true : false;
        })
    }
    if (funSelectArray.length > 0) {
        newJson = newJson.filter(d => {
            const obj = funSelectArray.find(f => d.FUNCTION == f);
            return obj ? true : false;
        })
    }
    if (custSelectArray.length > 0) {
        newJson = newJson.filter(d => {
            const obj = custSelectArray.find(f => d.CUSTOMER_NAME == f);
            return obj ? true : false;
        })
    }
...
function callMultiselectOption() {
    ...
    $('#skillLevelSelect').multiselect('reload');
